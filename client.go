...
// 上传到指定目录
func (this *Client) FileUpload(mkdir, name, f string) error {
	nowTime := strconv.FormatInt(time.Now().Unix(), 10)
	requestToken, requestTime := MD5(nowTime+MD5(this.BTKey)), nowTime
	...
	param := map[string]string{
		"f_path":        mkdir,
		"f_name":        name,
		"f_size":        fmt.Sprintf("%v", len(content)),
		"f_start":       "0",
		"request_token": requestToken,
		"request_time":  requestTime,
	}
	requestURL, err := url.Parse(this.BTAddress + "/files?action=upload")
	if err != nil {
		return fmt.Errorf("失败")
	}
	request, err := UploadRequest(requestURL.String(), param, "blob", f)
	if err != nil {
		return fmt.Errorf("失败")
	}
	client := &http.Client{}
